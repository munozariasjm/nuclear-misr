...
c Calculation of binding energy E (nx neutrons,nz protons)
     ...
     &          onp(0:8,2,2),oei[j](2),dei[j](2),nn(2),noc(18,2),pp(2),y(2)
...
c--------
        ...
        if(i2.ne.i)then            !     ssh j
          oei[j](j)=moc+ip*(ip-1)     !       nb of nucleons in last EI shell
          dei[j](j)=ip*(ip+1)+2       !       size of the EI shell
        else                       !     ssh r
          oei[j](j)=moc-ju            !       nb of nucleons in last EI shell
          dei[j](j)=(ip+1)*(ip+2)+2   !       size of the EI shell
        endif
        qx(j)=oei[j](j)*(dei[j](j)-oei[j](j)-ju)/dei[j](j)  ! n*(D-n)/D        S3(j)
        dx(j)=qx(j)*(2*oei[j](j)-dei[j](j))           ! n*(D-n)*(2n-D)/D  Q
        if(ndef.eq.2)qx(j)=qx(j)/sqrt(dei[j](j))   ! scaling for deformed
c--------
...
def dz_be(nx, nz):
    ...
    z2 = nz * (nz - 1)
    dyda = np.zeros(10)
    dyda[0] = (-z2 + 0.76 * z2 ** (2.0 / 3.0)) / rc  # Coulomb energy
    oei = np.zeros(2)
    dei = np.zeros(2)
    ...
    for ndef in range(2):  # beginning of main loop
        ju = 0
        if ndef == 1:
            ju = 4  # nucleons associated to deform.
            dyda[1:] = 0
        for j in range(2):  # beginning of loop over N and Z
            noc = np.zeros((19, 3))
            onp = np.zeros((9, 3, 3))
            n2 = np.zeros(2)
            n2[j] = 2 * (nn[j] // 2)  # for pairing calculation
            ...
            if ndef == 1:
                qx /= math.sqrt(dei[j])  # scaling for deformed
            # Amplitudes
            for k in range(imax):
                ip = k // 2
                fact = math.sqrt((ip + 1.0) * (ip + 2.0))
                onp[ip, 0, j] += noc[i, j] / fact  # for FM term
                vm = -1
                if (2 * (k // 2)) == k:
            ...
            op = np.zeros(2)
            os = np.zeros(2)
            for ip in range(ipm + 1):  # FM and SO terms
                pi = ip
                den = ((pi + 1) * (pi + 2)) ** (3.0 / 2.0)
                op[j] += onp[ip, 0, j]  # FM
                os[j] += onp[ip, 1, j] * (1 + onp[ip, 0, j]) * (pi * pi / den)  # SO
        ...
        if ndef == 1:
            dyda[5] = dx[0] + dx[1]
            dyda[6] = -dyda[5] / ra
            px = math.sqrt(pp[0]) + math.sqrt(pp[1])
            dyda[7] = qx[0] * qx[1] * (2**px)
        else:
            dyda[8] = qx[0] * qx[1]
        dyda[5] = t * (1 - t) / (a * ra**3) + dyda[5]
        if n2[0] != nn[0] and n2[1] != nn[1]:
            dyda[9] = t / a
        if nx > nz:
            if n2[0] == nn[0] and n2[1] != nn[1]:
                dyda[9] = 1 - t / a
            ...
            if n2[0] != nn[0] and n2[1] == nn[1]:
                dyda[9] = 1 - t / a
        if n2[1] == nn[1] and n2[0] == nn[0]:
            dyda[9] = 2 - t / a
        for mss in range(1, 10):
            dyda[mss] /= ra
        for mss in range(10):
            y[ndef] += dyda[mss] * b[mss]
    de = y[1] - y[0]
    E = y[1]  # Binding Energy for def. nuclides
    if de <= 0 or nz <= 50:
        E = y[0]  # spherical nuclides
    return E
