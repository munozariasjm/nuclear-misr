...
class SrBe:
    def __init__(self, base_model: str = "base") -> None:
        self.exprs_list = [
            "(((Z + ((-1.1518949008311261 / h) * 0.9191416779446455)) + h) * (((32.42652864512878 - (K / h)) * S) - -16.68481023317041))",  # 1
            "(((((np.exp(1.1006874673013554) - ((K - 1.3719238061914094) - (S / 0.4559834159964229))) * ((Z - (-2.0129653786217006 / -0.149121978667067)) - 1.1006874673013554)) * ((np.log(K) - (S / 0.32947021405273286)) * 1.1264781107199664)) + (d - P)) + 0.3012755947167558)",  # 2
            "((((0.8667226017107358 ** x) * np.exp(1.0957980468921584)) - (np.exp(0.7030577891026748 - (((0.036946866702474115 * Z) ** Z) - ((-0.39995415163224035 * P) * ee))) - S)) + (((-0.12360962447332499 * P) / 0.2905011442378596) * np.log(0.036946866702474115 * Z)))",  # 3
            "((((np.log(h ** S) * t) * (np.exp(Z ** 1.1017534523017007) / np.exp(t))) / np.exp(Z)) - (0.29005199356401196 * (((np.exp((t - N) + K) * np.log(0.11228275004501485)) - ee) - 0.8482750079357783)))",  # 4
            "((((x * P) + N) * (P + (1.538872526208315e-5 * x))) * ((1.538872526208315e-5 * t) ** P))",  # 5
            "((oe / N) - np.exp(((((P ** K) - (P ** N)) * ((P + P) + 0.215478490301157)) - ((h - -0.4261022127969257) + eo)) * (K + (-1.20622071093187 - h))))",  # 6
            "((S * 1.3513717643571443) * ((((-1.7835130140476037 - h) * (0.3243294855565521 - S)) * ((np.log(0.8949329729059017) * (np.exp(P) + A)) + (np.exp(1.4593173482869688) / h))) + ((S * 1.3513717643571443) - P)))",  # 7
            "((((0.8010663851875898 ** N) * (A + ((A / h) - x))) - 0.111962577802992) * ((((P / np.exp(0.5618598224805517)) - S) - (0.8010663851875898 ** oe)) - S))",  # 8
